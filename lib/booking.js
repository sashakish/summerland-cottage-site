...
function s3Location() {
  ...
  return { bucket, key: process.env.BOOKING_CONFIG_S3_KEY || 'calendar/bookings.json' };
}
...
async function loadS3Config() {
  ...
  const result = await s3.send(new GetObjectCommand({ Bucket: location.bucket, Key: location.key }));
  const text = await result.Body.transformToString();
  return JSON.parse(text);
}

async function saveS3Config(config) {
  const location = s3Location();
...
export async function saveConfig(config) {
  if (await saveS3Config(config)) return;
  await fs.writeFile(configPath(), `${JSON.stringify(config, null, 2)}\n`);
}

export async function appendConfirmedBooking({ checkIn, checkOut, source = 'direct-site', note = 'Direct site booking', stripeSessionId = '' }) {
  if (!checkIn || !checkOut) throw new Error('Missing booking dates.');
  const config = await loadConfig();
  const blockedDates = config.blockedDates || [];
  const duplicate = blockedDates.some(b => b.start === checkIn && b.end === checkOut && b.stripeSessionId === stripeSessionId);
  if (!duplicate) {
    blockedDates.push({ start: checkIn, end: checkOut, source, note, stripeSessionId });
    config.blockedDates = blockedDates.sort((a, b) => a.start.localeCompare(b.start));
